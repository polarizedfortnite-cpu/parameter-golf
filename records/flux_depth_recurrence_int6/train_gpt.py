...
class Hyperparameters:
    ...
    muon_weight_decay = float(os.environ.get("MUON_WD", 0.01))
    beta1 = float(os.environ.get("BETA1", 0.9))
    beta2 = float(os.environ.get("BETA2", 0.95))
    adam_eps = float(os.environ.get("ADAM_EPS", 1e-8))
    grad_clip_norm = float(os.environ.get("GRAD_CLIP_NORM", 0.0))

    # SwiGLU MLP: uses silu(gate(x))*up(x) instead of relu^2.
    use_swiglu = bool(int(os.environ.get("USE_SWIGLU", "0")))
    # SmearGate: blend each token embedding with its predecessor.
    use_smear_gate = bool(int(os.environ.get("USE_SMEAR_GATE", "0")))

    # Quantization: QUANT_BITS controls the effective bit width for compression.
    # int8=[-127,127], int7=[-63,63], int6=[-31,31], int5=[-15,15]
    quant_bits = int(os.environ.get("QUANT_BITS", 8))

    # QAT: Quantization-Aware Training with STE. Activate in the last qat_fraction of training.
    # Reduces LR by qat_lr_scale when QAT activates. Set QAT_FRAC=0 to disable.
    qat_fraction = float(os.environ.get("QAT_FRAC", 0.25))
    qat_lr_scale = float(os.environ.get("QAT_LR_SCALE", 0.5))

    # Test-time training (LoRA) hyperparameters.
    ttt_lora_rank = int(os.environ.get("TTT_LORA_RANK", 8))
    ttt_lora_lr = float(os.environ.get("TTT_LORA_LR", 0.01))
    ttt_chunk_size = int(os.environ.get("TTT_CHUNK_SIZE", 64))  # scoring stride
    ttt_train_chunk_size = int(os.environ.get("TTT_TRAIN_CHUNK_SIZE", 256))  # train every N tokens
...
def quantize_float_tensor(t: Tensor, quant_max: int = 127) -> tuple[Tensor, Tensor]:
    t32 = t.float()
    if t32.ndim == 2:
        clip_abs = (
        ...
        scale = (clip_abs / float(quant_max)).clamp_min(1.0 / float(quant_max))
        q = torch.clamp(torch.round(clipped / scale[:, None]), -quant_max, quant_max).to(torch.int8).contiguous()
        return q, scale.to(dtype=INT8_PER_ROW_SCALE_DTYPE).contiguous()

    clip_abs = float(torch.quantile(t32.abs().flatten(), INT8_CLIP_Q).item()) if t32.numel() else 0.0
    scale = torch.tensor(clip_abs / float(quant_max) if clip_abs > 0 else 1.0, dtype=torch.float32)
    q = torch.clamp(torch.round(torch.clamp(t32, -clip_abs, clip_abs) / scale), -quant_max, quant_max).to(torch.int8).contiguous()
    return q, scale

def _ste_quantize_weight(w: Tensor, quant_max: int) -> Tensor:
    """Fake-quantize a 2D weight for QAT: round to int grid, STE for backward."""
    scale = w.abs().amax(dim=-1, keepdim=True).clamp_min(1e-8) / quant_max
    w_q = (w / scale).round().clamp(-quant_max, quant_max) * scale
    return w + (w_q - w).detach()  # STE: forward uses quantized, backward uses original

def apply_qat_ste(model: nn.Module, quant_max: int) -> None:
    """Apply STE fake-quantization to all 2D weight parameters (in-place, reversible)."""
    for name, p in model.named_parameters():
        if p.ndim == 2 and p.numel() > INT8_KEEP_FLOAT_MAX_NUMEL and "tok_emb" not in name:
            p.data = _ste_quantize_weight(p.data, quant_max)
...
def quantize_state_dict_int8(state_dict: dict[str, Tensor], quant_bits: int = 8):
    ...
    for name, tensor in state_dict.items():
        ...
        # Keep tied embedding in fp16 — quantization errors compound because it's
        # used for both input embeddings and output logits.
        ...
        q, s = quantize_float_tensor(t, quant_max=quant_max)
        if s.ndim > 0:
...
class SwiGLUMLP(nn.Module):
    # SwiGLU: down(silu(gate(x)) * up(x)). Uses hidden = dim * mlp_mult * 2 // 3
    # so that total params match relu^2 MLP at the same mlp_mult.
    def __init__(self, dim: int, mlp_mult: int):
        super().__init__()
        hidden = dim * mlp_mult * 2 // 3
...
class GPT(nn.Module):
    def __init__(
        ...
        if use_smear_gate:
            self.smear_gate = nn.Parameter(torch.full((model_dim,), -2.0))  # sigmoid(-2)≈0.12 → mostly self
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor, lora=None) -> Tensor:
        ...
        if self.use_smear_gate:
            ...
            x_prev = F.pad(x[:, :-1], (0, 0, 1, 0))  # shift right, first position gets zero
            x = (1.0 - gate) * x + gate * x_prev
        x0 = x
        skips: list[Tensor] = []
...
def eval_val_ttt_lora(
    ...
    chunk_size = args.ttt_chunk_size  # scoring stride (e.g. 64)
    train_chunk_size = args.ttt_train_chunk_size  # training stride (e.g. 256)
    train_every = max(1, train_chunk_size // chunk_size)  # train every N chunks
    ...
    for bi in range(0, len(rank_docs), batch_size):
        ...
        for ci in range(max_nc):
            ...
            # Train: one Adam step on LoRA using the last train_chunk_size tokens
            if needs_train:
...
def main() -> None:
    ...
    log0(f"model_params:{n_params} dim:{args.model_dim} layers:{args.num_layers} mlp_mult:{args.mlp_mult} swiglu:{args.use_swiglu} smear:{args.use_smear_gate} quant:{args.quant_bits} qat:{args.qat_fraction}")
    ...
    qat_active = False
    qat_start_step = int(args.iterations * (1.0 - args.qat_fraction)) if args.qat_fraction > 0 else args.iterations + 1
    qat_quant_max = (1 << (args.quant_bits - 1)) - 1
    log0(f"qat_start_step:{qat_start_step} qat_fraction:{args.qat_fraction} qat_lr_scale:{args.qat_lr_scale}")
    ...
    while True:
        ...
        # Activate QAT after qat_start_step
        if not qat_active and step >= qat_start_step:
            qat_active = True
            log0(f"qat_activated step:{step} quant_bits:{args.quant_bits} quant_max:{qat_quant_max}")

        # QAT: save original weights and fake-quantize (STE)
        qat_originals: dict[str, Tensor] = {}
        if qat_active:
            for name, p in base_model.named_parameters():
                if p.ndim == 2 and p.numel() > INT8_KEEP_FLOAT_MAX_NUMEL and "tok_emb" not in name:
                    qat_originals[name] = p.data.clone()
                    s = p.data.abs().amax(dim=-1, keepdim=True).clamp_min(1e-8) / qat_quant_max
                    p.data = (p.data / s).round().clamp(-qat_quant_max, qat_quant_max) * s
        ...
        # QAT: restore original weights (gradients stay on p.grad via STE)
        if qat_originals:
            for name, p in base_model.named_parameters():
                if name in qat_originals:
                    p.data = qat_originals[name]
        ...
        effective_scale = scale * (args.qat_lr_scale if qat_active else 1.0)
        for opt in optimizers:
            for group in opt.param_groups:
    ...
    if master_process:
        ...
        quant_max = (1 << (args.quant_bits - 1)) - 1
        log0(
            f"Serialized model int{args.quant_bits}+zstd22: {quant_file_bytes} bytes "
            f"(quant_max:{quant_max} payload:{quant_stats['int8_payload_bytes']} raw_torch:{quant_raw_bytes} payload_ratio:{ratio:.2f}x)"
        )
        log0(f"Total submission size int{args.quant_bits}+zstd22: {quant_file_bytes + code_bytes} bytes")
...
class Hyperparameters:
    ...
    num_passes: int = int(os.environ.get("NUM_PASSES", 1))
    # === NEW: SwiGLU ===
...
# === NEW: Sliding window evaluation ===
def eval_val_sliding(args, model, rank, world_size, device, stride, val_tokens, base_bytes_lut, has_leading_space_lut, is_boundary_token_lut, log_fn=None):
    """Sliding window eval: slide by stride tokens, score only the last stride per window."""
    seq_len = args.train_seq_len
    n = val_tokens.numel()
    max_start = n - seq_len - 1
    starts = list(range(0, max_start + 1, stride))
    rank_starts = starts[rank::world_size]
    batch_size = max(args.val_batch_size // (seq_len * 8), 1)
    ...
    # Get the base (uncompiled, unwrapped) model for direct forward
    base = model.module if hasattr(model, 'module') else model
    base = base._orig_mod if hasattr(base, '_orig_mod') else base

    base.eval()
    with torch.inference_mode():
        total_batches = (len(rank_starts) + batch_size - 1) // batch_size
        for batch_idx in range(total_batches):
            b_start = batch_idx * batch_size
            b_end = min(b_start + batch_size, len(rank_starts))
            batch_starts_list = rank_starts[b_start:b_end]

            x_list = [val_tokens[s : s + seq_len].to(torch.int64) for s in batch_starts_list]
            y_list = [val_tokens[s + 1 : s + seq_len + 1].to(torch.int64) for s in batch_starts_list]
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                ...
                per_token_loss = F.cross_entropy(
                    ...
                    y.reshape(-1),
                    reduction="none"
                ).reshape(bsz_local, seq_len)

            for i, s in enumerate(batch_starts_list):
                score_start = 0 if s == 0 else (seq_len - stride)
                n_scored = seq_len - score_start
                total_loss_sum += per_token_loss[i, score_start:].to(torch.float64).sum()
                total_scored_tokens += n_scored
                global_pos = s + score_start
                prev_ids = val_tokens[global_pos : global_pos + n_scored].to(device=device, dtype=torch.int64)
                tgt_ids = val_tokens[global_pos + 1 : global_pos + n_scored + 1].to(device=device, dtype=torch.int64)
                token_bytes = base_bytes_lut[tgt_ids].to(dtype=torch.int16)
                token_bytes += (has_leading_space_lut[tgt_ids] & ~is_boundary_token_lut[prev_ids]).to(dtype=torch.int16)
                total_bytes += token_bytes.to(torch.float64).sum()

            if log_fn is not None and (batch_idx == 0 or (batch_idx + 1) % 50 == 0 or batch_idx + 1 == total_batches):
                log_fn(f"sliding_eval_progress:{batch_idx + 1}/{total_batches}")
...
# === NEW: SwiGLU MLP ===
class SwiGLUMLP(nn.Module):
    """SwiGLU: down(silu(gate(x)) * up(x)). Iso-param with relu^2 MLP."""
    def __init__(self, dim, mlp_mult):
        super().__init__()
        hidden = dim * mlp_mult * 2 // 3
        self.gate = CastedLinear(dim, hidden, bias=False)
...
# === MODIFIED: GPT with depth recurrence support ===
class GPT(nn.Module):
    def __init__(self, vocab_size, num_layers, model_dim, num_heads, num_kv_heads, mlp_mult,
        ...
        self.use_recurrence = num_unique_blocks > 0

        self.tok_emb = nn.Embedding(vocab_size, model_dim)

        if self.use_recurrence:
            self.blocks = nn.ModuleList([
                Block(model_dim, num_heads, num_kv_heads, mlp_mult, rope_base, qk_gain_init, use_swiglu)
                for _ in range(num_unique_blocks)
            ])
            self.skip_weights = nn.Parameter(torch.zeros(1, model_dim, dtype=torch.float32))
        else:
            self.num_encoder_layers = num_layers // 2
            self.num_decoder_layers = num_layers - self.num_encoder_layers
            self.num_skip_weights = min(self.num_encoder_layers, self.num_decoder_layers)
...
def main():
    ...
    # === MODIFIED: model construction with new params ===
    base_model = GPT(
        vocab_size=args.vocab_size, num_layers=args.num_layers, model_dim=args.model_dim,
        num_heads=args.num_heads, num_kv_heads=args.num_kv_heads, mlp_mult=args.mlp_mult,
        tie_embeddings=args.tie_embeddings, tied_embed_init_std=args.tied_embed_init_std,
        logit_softcap=args.logit_softcap, rope_base=args.rope_base, qk_gain_init=args.qk_gain_init,
        num_unique_blocks=args.num_unique_blocks, num_passes=args.num_passes, use_swiglu=args.use_swiglu,
    ).to(device).bfloat16()
    for module in base_model.modules():
    ...
    n_params = sum(p.numel() for p in base_model.parameters())
    recur_info = f" unique_blocks:{args.num_unique_blocks} passes:{args.num_passes} eff_depth:{args.num_unique_blocks * args.num_passes}" if args.num_unique_blocks > 0 else ""
    log0(f"model_params:{n_params} use_swiglu:{args.use_swiglu}{recur_info}")
    log0(f"world_size:{world_size} grad_accum_steps:{grad_accum_steps}")
    ...
    # === MODIFIED: Use sliding window eval if stride > 0 ===
    if args.eval_stride > 0:
        log0(f"sliding_window_eval stride:{args.eval_stride} seq_len:{args.train_seq_len}")
        q_val_loss, q_val_bpb = eval_val_sliding(
            args, model, rank, world_size, device, args.eval_stride,
