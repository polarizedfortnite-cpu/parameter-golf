"""
Parameter Golf submission: Depth Recurrence + SwiGLU + Sliding Window Eval
Based on openai/parameter-golf baseline train_gpt.py with three additions:
1. Depth recurrence (NUM_UNIQUE_BLOCKS unique blocks looped NUM_PASSES times)
2. SwiGLU activation (iso-param replacement for relu^2 FFN)
3. Sliding window evaluation (EVAL_STRIDE tokens, scoring last stride per window)

Run: NUM_UNIQUE_BLOCKS=4 NUM_PASSES=3 MODEL_DIM=832 MLP_MULT=3 USE_SWIGLU=1 TRAIN_SEQ_LEN=2048 EVAL_STRIDE=64 torchrun --standalone --nproc_per_node=8 train_gpt.py
"""
...
def main():
    ...
    optimizer_muon = Muon(matrix_params, lr=args.matrix_lr, momentum=args.muon_momentum, backend_steps=args.muon_backend_steps, weight_decay=float(os.environ.get("MUON_WEIGHT_DECAY", "0.0")))
